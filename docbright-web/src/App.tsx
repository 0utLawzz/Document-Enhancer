...
/** Quick-access ink color palette (deeper tones for solid ink look) */
const INK_SWATCHES = [
  { label: 'Navy Blue',     hex: '#001f5c' },
  { label: 'Deep Red',      hex: '#8b0a1a' },
  { label: 'Forest Green',  hex: '#0f4a1c' },
  { label: 'Purple',        hex: '#4c0e7a' },
  { label: 'Teal',          hex: '#064e52' },
  { label: 'Brown',         hex: '#5c2e0a' },
  { label: 'Royal Blue',    hex: '#122b7a' },
  { label: 'Dark Orange',   hex: '#8a3a05' },
];
...
function App() {
  ...
  const addFiles = async (files: FileList | File[]) => {
    ...
    const pdfFiles = list.filter(
      (f) => f.type === 'application/pdf' || f.name.toLowerCase().endsWith('.pdf'),
    );
    if (!imageFiles.length && !pdfFiles.length) {
      setErrorMessage('Choose JPG, PNG, WEBP, PDF, or another supported file to begin.');
      return;
    }
    setIsAdding(true);
    setErrorMessage('');
    try {
      ...
      for (let index = 0; index < imageFiles.length; index += 1) {
        ...
        added.push({
          ...
          status: 'waiting',
          preset: batchPreset,
          inkColor: batchInkColor || undefined,
          width: size.width,
          height: size.height,
          size: file.size,
          rotation: 0,
      ...
      // PDFs → split each page into its own queue item (PDF → image)
      for (const file of pdfFiles) {
        const pages = await pdfToImages(file, { scale: 2 });
        const base = cleanName(file.name, 'PDF');
        for (const page of pages) {
          added.push({
            id: makeId(),
            name: pages.length > 1 ? `${base}_p${String(page.page).padStart(2, '0')}` : base,
            originalUri: page.uri,
            createdAt: new Date().toISOString(),
            status: 'waiting',
            preset: batchPreset,
            inkColor: batchInkColor || undefined,
            width: page.width,
            height: page.height,
            size: Math.round((page.uri.length * 3) / 4),
            rotation: 0,
          });
        }
      }
    ...
    <div className="app-shell">
      ...
      <main className="workspace">
        ...
        <div className="work-grid">
          <section className="main-column" aria-label="Document queue">
            <div className="panel">
              <div
                ...
                role="button"
                tabIndex={0}
                aria-label="Upload image or PDF files"
              ...
              >
                <div className="dropzone-inner">
                  ...
                  <p>
                    Drop images or PDFs. Multi-page PDFs are split into one queue item per page (PDF → image).
          ...
          <aside className="rail" aria-label="Enhancement controls">
            <section className="panel">
              <div className="panel-header"><h2>BATCH CONTROLS</h2></div>
              <div className="control-body">
                <div className="control-block">
                  <label className="section-label" htmlFor="batch-preset">Apply preset to all</label>
                  <select id="batch-preset" className="rail-select" value={batchPreset} data-testid="select-batch-preset" onChange={(e) => setBatchPreset(e.target.value as Preset)}>
                    {PRESETS.map((p) => <option value={p.name} key={p.name}>{p.icon} {p.name}</option>)}
                  </select>
                  <p>{PRESETS.find((p) => p.name === batchPreset)?.detail}</p>
                </div>

                <div className="control-block">
                  <label className="section-label" htmlFor="batch-ink-color">Batch ink color</label>
            ...
            <section className="privacy-panel" data-testid="panel-privacy">
              <div className="privacy-icon"><ShieldCheck size={14} /> Local privacy</div>
              <h3>NO CLOUD.<br />NO COPIES.</h3>
              <p>Images and PDFs are processed by your browser. The queue stays in local storage only. Nothing is uploaded.</p>
            </section>
        ...
        <footer className="app-footer">
          ...
          <span>v2.1 · {new Date().getFullYear()}</span>
        </footer>
      </main>
